...
def apply_image_filters(np_img, slide_name=None, info=None, save=False, display=False):
  ...
  Args:
    ...
    slide_name: The slide number (used for saving/displaying).
  ...
  """
  rgb = np_img
  save = display = False #disable intermediate save/ display 
  save_display(save, display, info, rgb, slide_name, 1, "Original", "rgb")
  #print(rgb.shape)
  mask_not_green = filter_green_channel(rgb,overmask_thresh=98.5)
  rgb_not_green = util.mask_rgb(rgb, mask_not_green)
  save_display(save, display, info, rgb_not_green, slide_name, 2, "Not Green", "rgb-not-green")

  mask_not_gray = filter_grays(rgb)
  rgb_not_gray = util.mask_rgb(rgb, mask_not_gray)
  save_display(save, display, info, rgb_not_gray, slide_name, 3, "Not Gray", "rgb-not-gray")

  mask_no_red_pen = filter_red_pen(rgb)
  rgb_no_red_pen = util.mask_rgb(rgb, mask_no_red_pen)
  save_display(save, display, info, rgb_no_red_pen, slide_name, 4, "No Red Pen", "rgb-no-red-pen")

  mask_no_green_pen = filter_green_pen(rgb)
  rgb_no_green_pen = util.mask_rgb(rgb, mask_no_green_pen)
  save_display(save, display, info, rgb_no_green_pen, slide_name, 5, "No Green Pen", "rgb-no-green-pen")

  mask_no_blue_pen = filter_blue_pen(rgb)
  rgb_no_blue_pen = util.mask_rgb(rgb, mask_no_blue_pen)
  save_display(save, display, info, rgb_no_blue_pen, slide_name, 6, "No Blue Pen", "rgb-no-blue-pen")
  ...
  save_display(save, display, info, rgb_gray_green_pens, slide_name, 7, "Not Gray, Not Green, No Pens",
               "rgb-no-gray-no-green-no-pens")

  # mask_remove_small = filter_remove_small_objects(mask_gray_green_pens, min_size=500, output_type="bool",overmask_thresh=99.99)
  # rgb_remove_small = util.mask_rgb(rgb, mask_remove_small)
  # save_display(save, display, info, rgb_remove_small, slide_name, 8,
...
def apply_filters_to_image(slide_name, save=True, display=False):
  ...
  filtered_np_img = apply_image_filters(np_orig, slide_name, info, save=save, display=display)
...
def save_display(save, display, info, np_img, slide_name, filter_num, display_text, file_text,
  ...
  if slide_name is None and filter_num is None:
    ...
    display_text = f"S_{slide_name}-{display_text}"
  elif slide_name is None:
    display_text = f"F_{filter_num}-{display_text}"
  else:
    display_text = f"S_{slide_name}_F:{filter_num}-{display_text}"
  if display:
    util.display_img(np_img, display_text)
  if save:
    save_filtered_image(np_img, slide_name, filter_num, file_text)
  if info is not None:
    info[slide_name] = (slide_name, filter_num, display_text, file_text, mask_percentage)
...
def save_filtered_image(np_img, slide_name, filter_num, filter_text):
  ...
  out_dir = f'workspace/filter_tmp/{slide_name}'
  os.makedirs(out_dir,exist_ok=True)
  filepath = f"{out_dir}/{filter_num}.jpeg"
...
def apply_filters_to_image_list(image_name_list, save, display):
  """
  Apply filters to a list of images.

  Args:
    image_name_list: List of image numbers.
    save: If True, save filtered images.
    display: If True, display filtered images to screen.
  ...
  """
  html_page_info = dict()
  for slide_name in image_name_list:
    _, info = apply_filters_to_image(slide_name, save=save, display=display)
    html_page_info.update(info)
  return image_name_list, html_page_info
...
def singleprocess_apply_filters_to_images(save=True, display=False, html=False, image_name_list=None):
  """
...
def multiprocess_apply_filters_to_images(save=True, display=False, html=False, image_name_list=None):
  """
  ...
  if image_name_list is not None:
    num_train_images = len(image_name_list)

  if num_processes > num_train_images:
    num_processes = num_train_images
  images_per_process = num_train_images / num_processes

  print("Number of processes: " + str(num_processes),"Number of training images: " + str(num_train_images))
  ...
  for num_process in range(1, num_processes + 1):
    ...
    if image_name_list is not None:
      sublist = image_name_list[start_index - 1:end_index]
...
# singleprocess_apply_filters_to_images(image_name_list=[2], display=True)

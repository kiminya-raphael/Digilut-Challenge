...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
  ...
  summary_title = "Slide {slide_name} Top Tile Summary:"
  summary_txt = summary_title + "\n" + summary_stats(tile_sum)

  draw.text((5, 5), summary_txt, SUMMARY_TITLE_TEXT_COLOR)
  ...
  for t in tiles_to_label:
    ...
    draw.text(((t.c_s + h_ds_offset), (t.r_s + v_ds_offset + z)), label, (0, 0, 0))

    draw.text(((t.c_s + h_offset), (t.r_s + v_offset + z)), label, SUMMARY_TILE_TEXT_COLOR)
  ...
  if save_summary:
    save_top_tiles_image(summary, slide_name)
...
def np_tile_stat_img(tiles):
  ...
  np_stats = np_text(tile_stats, font_size=14)
  return np_stats
...
def score_tiles(slide_name, np_img=None, dimensions=None, small_tile_in_tile=False):
  """
  ...
  if dimensions is None:
    #img_path = slide.get_filter_image_result(slide_name)
    o_h=h = np_img.shape[0]
    o_w=w = np_img.shape[1]
    #print('width height',o_w,o_h)
  else:
    o_w, o_h, w, h = dimensions

  row_tile_size = col_tile_size = TILE_SIZE_BASE

  num_row_tiles, num_col_tiles = get_num_tiles(h, w, row_tile_size, col_tile_size)

  tile_sum = TileSummary(slide_name=slide_name,
                         orig_w=o_w,
                         orig_h=o_h,
  ...
  for t in tile_indices:
    ...
    if(np.mean(np_tile)<MIN_MEAN_INFO):
      # score = color_factor= s_and_v_factor= quantity_factor = 0
      continue
    ...
    # if(np.mean(np_tile)==0):
    #   score = color_factor= s_and_v_factor= quantity_factor = 0
    #   continue
    # else:
    score, color_factor, s_and_v_factor, quantity_factor = score_tile(np_tile, t_p, slide_name, r, c)

    np_scaled_tile = np_tile if small_tile_in_tile else None
    tile = Tile(tile_sum, slide_name, np_scaled_tile, count, r, c, r_s, r_e, c_s, c_e, t_p, color_factor, s_and_v_factor, quantity_factor, score)
...
def score_tile(np_tile, tissue_percent, slide_name, row, col):
  """
...
def image_list_to_tiles(image_list, display=False, save_summary=True, save_data=True, save_top_tiles=True):
  """
  ...
  return image_list, tile_summaries_dict
...
def pil_text(text, w_border=TILE_TEXT_W_BORDER, h_border=TILE_TEXT_H_BORDER,
  ...
  x, y = ImageDraw.Draw(Image.new("RGB", (1, 1), background)).textsize(text)
  image = Image.new("RGB", (x + 2 * w_border, y + 2 * h_border), background)
  draw = ImageDraw.Draw(image)
  draw.text((w_border, h_border), text, text_color)
  return image


def np_text(text, w_border=TILE_TEXT_W_BORDER, h_border=TILE_TEXT_H_BORDER, 
            font_size=TILE_TEXT_SIZE, text_color=TILE_TEXT_COLOR, background=TILE_TEXT_BACKGROUND_COLOR):
  """
  ...
  """
  pil_img = pil_text(text, w_border, h_border, font_size,
...
class TileSummary:
  ...
  def __init__(self, slide_name, orig_w, orig_h, orig_tile_w, orig_tile_h, scaled_w, scaled_h, scaled_tile_w,
  ...
  def display_summaries(self):
    ...
    """
    summary_and_tiles(self.slide_name, display=True, save_summary=False, save_data=False, save_top_tiles=False)
...
class Tile:
  ...
  def __init__(self, tile_summary, slide_name, np_scaled_tile, tile_num, r, c, r_s, r_e, c_s, c_e, 

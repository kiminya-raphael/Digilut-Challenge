...
RAW_IMG_DIR = os.path.join(os.environ['DIR_RAW_DATA'] ,'images')
...
def get_slide(slide_name):
    ...
    #TODO - pyvips->numpy for speed
    # try:
    #slide = pyvips.Image.new_from_file(f'{RAW_IMG_DIR}/{slide_name}.{SLIDE_EXT}',page=BASE_PAGE).numpy()
    #print(f'reading', f'{RAW_IMG_DIR}/{slide_name}.{SLIDE_EXT}')
    slide = None
    try:
        slide = sk.ImageCollection(f'{RAW_IMG_DIR}/{slide_name}.{SLIDE_EXT}')[BASE_PAGE]
